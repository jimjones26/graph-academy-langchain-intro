...
def get_movie_plots(limit=None):
    ...
    movies, summary, keys = driver.execute_query(query)
...
def generate_embeddings(file_name, limit=None):
    ...
    for movie in movies:
        ...
        response = llm.embeddings.create(input=plot, model="text-embedding-ada-002")

        output_plot.writerow(
            {"movieId": movie["movieId"], "embedding": response.data[0].embedding}
        )
